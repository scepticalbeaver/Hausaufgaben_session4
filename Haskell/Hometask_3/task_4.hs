...
findMaxNeighbours (x : xs) =
    snd (maximumBy maxFst (zip (zipWith(+) (x : xs) (xs ++ [0])) [1..])) where
...
test =
    ...
    && findMaxNeighbours [1, 2, 3, 12, 0, 12, 0, 12, 3, 2, 1] == 3
    && findMaxNeighbours [1, 5, 7, 4, 13, 4] == 4
    && findMaxNeighbours [1, 5, 7, 4, 213, 31] == 5
